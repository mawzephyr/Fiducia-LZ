...
async def generate_custom_baseline_pdf(
    ...
    """
    Generate a PDF baseline report for specific assets on a specific date.
    """
    ...
    header_content = [
        [Paragraph("CIP-010 Custom Baseline Report", title_style), ""]
    ]
    ...
    doc.build(story)
    buffer.seek(0)
    filename = f"CIP010_Custom_Baseline_{check_date.isoformat()}.pdf"
...
def _generate_investigation_closure_pdf(investigations_closed: list, closure_time: datetime, closed_by: str) -> io.BytesIO:
    ...
    Creates a professional PDF with Fiducia logo and comprehensive investigation details.
    """
    from reportlab.lib.pagesizes import letter
    from reportlab.lib import colors
    from reportlab.lib.styles import getSampleStyleSheet, ParagraphStyle
    ...
    if os.path.exists(logo_path):
        try:
            # Logo is square (960x960), preserve aspect ratio
    ...
    # Title
    story.append(Paragraph("INVESTIGATION CLOSURE SUMMARY", title_style))
    ...
    if all_tickets:
        overview_data.append(["Related Tickets:", ", ".join(sorted(all_tickets))])
    ...
    for idx, inv in enumerate(investigations_closed, 1):
        ...
        story.append(Paragraph(f"Investigation #{idx}: {inv.get('name', 'Unknown Asset')}", section_header_style))
        story.append(Spacer(1, 0.1*inch))

        # Asset Information
        story.append(Paragraph("Asset Information", subsection_style))

        asset_data = [
            ["Asset Name:", inv.get("name", "N/A")],
        ]
        if inv.get("fqdn"):
            asset_data.append(["FQDN:", inv["fqdn"]])
        if inv.get("group"):
            asset_data.append(["Team/Group:", inv["group"]])
        asset_data.append(["Asset ID:", str(inv.get("id", "N/A"))])

        asset_table = Table(asset_data, colWidths=[1.5*inch, 5*inch])
        ...
        # Timeline
        story.append(Paragraph("Investigation Timeline", subsection_style))

        timeline_data = []
        if inv.get("investigation_started_at"):
            try:
                start_dt = datetime.fromisoformat(inv["investigation_started_at"].replace('Z', '+00:00'))
                timeline_data.append(["Investigation Opened:", start_dt.strftime("%Y-%m-%d %H:%M:%S UTC")])
            except:
                timeline_data.append(["Investigation Opened:", inv["investigation_started_at"]])

        timeline_data.append(["Investigation Closed:", closure_time.strftime("%Y-%m-%d %H:%M:%S UTC")])

        if inv.get("investigation_duration_days") is not None:
            duration = inv["investigation_duration_days"]
            if duration == 0:
                duration_str = "Same day resolution"
            elif duration == 1:
                duration_str = "1 day"
            else:
                duration_str = f"{duration} days"
            timeline_data.append(["Duration:", duration_str])

            # Compliance status
            if duration <= 35:
                status_str = "WITHIN 35-DAY WINDOW"
                status_color = colors.HexColor('#059669')
            else:
                status_str = f"EXCEEDED 35-DAY WINDOW (by {duration - 35} days)"
                status_color = colors.HexColor('#dc2626')
            timeline_data.append(["Compliance Status:", status_str])

        timeline_data.append(["Changes Resolved:", str(inv.get("changes_finalized", 0))])
        ...
        if inv.get("investigation_notes"):
            story.append(Paragraph("Investigation Notes", subsection_style))
            # Handle long notes with proper wrapping
            notes_text = inv["investigation_notes"].replace('\n', '<br/>')
            story.append(Paragraph(notes_text, notes_style))
            story.append(Spacer(1, 0.15*inch))

        # Changes Resolved
        story.append(Paragraph("Changes Resolved", subsection_style))

        changes = inv.get("changes", [])
        if changes:
            for change_idx, change in enumerate(changes, 1):
                story.append(Paragraph(f"<b>Change {change_idx} of {len(changes)}</b>", normal_style))

                change_data = [
                    ["Field Path:", change.get("field", "N/A")],
                    ["Change Type:", change.get("change_type", "N/A").upper().replace("_", " ")],
                ]

                if change.get("ticket_number"):
                    change_data.append(["Ticket #:", change["ticket_number"]])

                # Handle array_modified vs other change types
                change_type = change.get("change_type", "").lower()
                
                if change_type == "array_modified":
                    # For array changes, show items added/removed
                    items_added = change.get("items_added") or []
                    items_removed = change.get("items_removed") or []
                    
                    if items_added:
                        change_data.append(["", ""])  # spacer row
                        added_header = f"ADDED ({len(items_added)} items):"
                        change_data.append([added_header, ""])
                        for item in items_added:
                            if isinstance(item, dict):
                                item_str = ", ".join(f"{k}: {v}" for k, v in item.items())
                                change_data.append(["  +", f"{{ {item_str} }}"])
                            else:
                                change_data.append(["  +", str(item)])
                    
                    if items_removed:
                        change_data.append(["", ""])  # spacer row
                        removed_header = f"REMOVED ({len(items_removed)} items):"
                        change_data.append([removed_header, ""])
                    ...
                    # Show old/new values for non-array changes
                ...
                if change.get("approved_at"):
                    try:
                        ...
                        change_data.append(["Reviewed At:", approved_dt.strftime("%Y-%m-%d %H:%M:%S UTC")])
                    except:
                        change_data.append(["Reviewed At:", change["approved_at"]])

                if change.get("resolution_notes"):
                    change_data.append(["Notes:", change["resolution_notes"]])

                change_data.append(["Status:", "APPROVED - PROMOTED TO BASELINE"])
            ...
            story.append(Paragraph("No change details available.", normal_style))

        # Resolution Summary
        story.append(Spacer(1, 0.1*inch))
        story.append(Paragraph("Resolution Summary", subsection_style))

        resolution_items = [
            "Investigation has been completed and closed.",
            "All configuration changes have been reviewed and approved.",
            "Approved changes have been promoted to the official baseline.",
            "Asset has been restored to COMPLIANT status.",
            "CIP-010 compliance requirements have been satisfied."
        ]
...
def _generate_promotion_pdf(finalized_assets: list, promotion_time: datetime, promoted_by: str) -> io.BytesIO:
    ...
    for idx, asset in enumerate(finalized_assets, 1):
        ...
        if asset_info:
            asset_table = Table(asset_info, colWidths=[1.2*inch, 5.3*inch])
            asset_table.setStyle(TableStyle([
                ('FONTNAME', (0, 0), (0, -1), 'Helvetica-Bold'),
                ('FONTSIZE', (0, 0), (-1, -1), 9),
                ('TEXTCOLOR', (0, 0), (0, -1), colors.HexColor('#6b7280')),
        ...
        for i, change in enumerate(changes, 1):
            story.append(Paragraph(f"<b>Change {i}: {change.get('field', 'Unknown Field')}</b>", normal_style))
            
            change_data = []
            change_data.append(["Type:", change.get('change_type', 'modified').upper()])
            ...
            if change.get('ticket_number'):
                change_data.append(["Ticket #:", change['ticket_number']])
            if change.get('approved_by'):
                change_data.append(["Approved By:", change['approved_by']])
            if change.get('detected_at'):
                change_data.append(["Detected:", change['detected_at']])
            if change.get('time_to_close_days') is not None:
                ...
                change_data.append(["Time to Close:", status])
            if change.get('resolution_notes'):
                notes = change['resolution_notes']
                if len(notes) > 60:
                    notes = notes[:57] + "..."
                change_data.append(["Notes:", notes])
